...
import os
import sys
import logging # Import logging for error messages
...
project_root_dir = os.path.dirname(config_dir) # Go up from utils to aed
metadata_path = os.path.join(project_root_dir, 'metadata', 'class_labels_indices.csv')
...
try:
    ...
    logging.error(f"Error: Metadata file not found at {metadata_path}. Please ensure 'class_labels_indices.csv' exists in the 'metadata' directory.")
    # Set default empty values to avoid further errors
    labels = []
    ids = []
    classes_num = 0
    lb_to_ix = {}
    ...
    logging.error(f"An unexpected error occurred while loading metadata from {metadata_path}: {e}")

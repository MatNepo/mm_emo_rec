    ...
    def process_video(self, input_video_path, output_video_path):
        ...
        try:
            while cap.isOpened():
                ...
                # Update statistics
                self.update_statistics(len(merged_boxes) if 'merged_boxes' in locals() else 0, 
                                     frame_emotions, processing_time)
                ...
                for (x, y, w, h), emotion in zip(merged_boxes, frame_emotions):
                    ...
                    text_x = x
                    text_y_baseline = y - padding # Try to place text baseline 5 pixels above bbox top

                    # Adjust text_y if it goes off screen at the top
                    if (text_y_baseline - text_h - baseline) < 0: # If top of text goes off screen
                        text_y_baseline = y + text_h + padding # Place text baseline below bbox top, inside
            ...
            # Return results in the format expected by results.json (raw counts)
            return {
